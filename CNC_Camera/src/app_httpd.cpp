...
#include "driver/ledc.h"  // Assure-toi d'inclure cette bibliothèque
...
void setupLedFlash(int pin) {
    #if CONFIG_LED_ILLUMINATOR_ENABLED
        // Définir la fréquence (5000 Hz) et la résolution (8 bits) pour le canal PWM
        int channel = 0;  // Choisis un canal PWM (0 à 15)
        int freq = 5000;   // Fréquence en Hz
        int resolution = 8;  // Résolution de 8 bits

        // Configure le canal PWM
        ledcSetup(channel, freq, resolution);

        // Attacher le canal PWM à la broche spécifiée
        ledcAttachPin(pin, channel);

        // Initialiser la PWM à une valeur de 0 (éteindre la LED)
        ledcWrite(channel, 0);  // Valeur 0 pour éteindre, 255 pour allumer la LED à pleine intensité
    #else
